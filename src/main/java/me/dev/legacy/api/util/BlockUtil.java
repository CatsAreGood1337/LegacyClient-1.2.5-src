...
import me.dev.legacy.api.util.Combat.HoleFillUtil;
import me.dev.legacy.api.util.Minecraft.RotationUtil;
...
import static me.dev.legacy.api.util.Minecraft.RotationUtil.getLegitRotations;
...
public class BlockUtil
    ...
    public static void placeBlock(final BlockPos pos) {
        for (final EnumFacing side : EnumFacing.VALUES) {
            ...
            if (neighborState.getBlock().canCollideCheck(neighborState, false)) {
                final boolean sneak = !PlayerUtil.isShifting() && neighborState.getBlock().onBlockActivated(mc.world, pos, mc.world.getBlockState(pos), mc.player, EnumHand.MAIN_HAND, side, 0.5f, 0.5f, 0.5f);
                if (sneak) {
    ...
    public static void placeCrystalOnBlock1(final BlockPos pos, final EnumHand hand) {

    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            if (Legacy.friendManager.getFriends().isEmpty()) {
                ...
                String f = "Friends: ";
                for (FriendManager.Friend friend : Legacy.friendManager.getFriends()) {
                    try {
        ...
        if (commands.length == 2) {
            ...
            FriendCommand.sendMessage(commands[0] + (Legacy.friendManager.isFriend(commands[0]) ? " is friended." : " isn't friended."));
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
